...
class MultiSearchStrategy(object):
    ...
    def init(self, start_sample, maxeval, numeval, issync, fhat, avoid=None):
        ...
        for i in range(len(self.search_strategies)):
            self.search_strategies[i].init(self.proposed_points, maxeval, numeval, issync, fhat, avoid)
...
class CandidateSRBF(object):
    ...
    def init(self, start_sample, maxeval, numeval, issync, fhat, avoid=None):
        ...
        self.fhat = fhat
        self.avoid = avoid
        self.n0 = start_sample.shape[0] + numeval
...
class CandidateDYCORS(CandidateSRBF):
    ...
    def _generate_cand(self, scalefactors, xbest, subset):
        initeval = len(self.proposed_points)
        ddsprob = self.probfun(self.numeval + initeval, self.maxeval, self.n0)
...
class MultiStartGradient(object):
    ...
    def next(self):
        ...
        fvals = np.zeros(self.numrestarts)
        xvals = np.zeros((self.numrestarts, self.data.dim))
        dists = np.zeros(self.numrestarts)
        ...
        for i in range(self.numrestarts):
            ...
            xx = to_unit_box(res.x, self.data)
            dist = np.atleast_2d(np.min(
                scp.distance.cdist(self.proposed_points, np.atleast_2d(xx)), axis=1)).T
        ...
        if dists.max() > self.dtol:
            ...
            fbest = np.inf
            for i in range(self.numrestarts):
                if dists[i] > self.dtol and fvals[i] < fbest:
                    xbest = xvals[i, :]
                    fbest = fvals[i]
        else:
            ...
            d = 0.0
            while d < self.dtol:
                xbest = xvals[np.argmin(fvals), :] + self.dtol * np.random.randn(1, self.data.dim)
                xbest = np.maximum(xbest, np.zeros(self.data.dim))
                xbest = np.minimum(xbest, np.ones(self.data.dim))
                d = np.atleast_2d(np.min(
                    scp.distance.cdist(self.proposed_points, np.atleast_2d(xbest)), axis=1)).T.min()
